...
from torchsummary import (
    summary,
)  # 计算模型参数,查看模型结构, pip install torchsummary -i https://mirrors.aliyun.com/pypi/simple/
...
class ModelDemo(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.softmax(
            self.output(torch.relu(self.linear2(torch.sigmoid(self.linear1(x))))), dim=1
        )
...
def train():
    ...
    # 4. 计算 和 查看模型参数.
    print("=================== 计算模型参数 ===================")
    # 参1: (人工神经网络ANN)模型对象, 参2: 输入数据维度(5行3列)
